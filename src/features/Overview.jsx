...
export default function Overview({state,onNavigate,healthCard,request}){
  ...
  return <div className="m-overview">
    ...
    <div className="m-chart-grid">
      ...
      <section className="m-panel m-goal-panel" data-tour-target="plan-ahorro-card"><div className="m-panel-heading"><span className="m-eyebrow">Tu meta</span><button className="m-text-button" onClick={()=>onNavigate('goals')}>Ver metas ↗</button></div>{goal?<><h2>{goal.name}</h2><div className="m-goal-ring" style={{'--progress':`${progress*100}%`}} role="img" aria-label={`${Math.round(progress*100)} por ciento del objetivo`}><div><strong>{Math.round(progress*100)}%</strong><span>del objetivo</span></div></div><strong className="m-goal-money">{fmt(goalCash(goal))}</strong><p className="m-muted">reservados · objetivo {fmt(goal.target)}</p>{goalProgress(goal)>goalCash(goal)&&<p className="m-muted">Pagado: {fmt(goalProgress(goal)-goalCash(goal))}</p>}{invested>0&&<p className="m-muted">Inversiones vinculadas: {fmt(invested)}</p>}<p className="m-goal-note">La reserva conserva su propósito.<br/>Los pagos se muestran por separado.</p></>:<div className="m-empty"><h2>Dale un propósito</h2><p>Elegí tu primera meta y empezá a reservar.</p><button className="m-secondary" onClick={()=>onNavigate('goals',{action:'new'})}>Crear una meta</button></div>}</section>
    </div>
    <section ref={changesRef} className="m-changes" data-tour-target="generar-resumen"><div className="m-panel-heading"><div><h2>Qué cambió</h2><p className="m-muted">Comparación de períodos equivalentes</p></div><button className="m-text-button" onClick={()=>onNavigate('analytics')}>Ver analíticas ↗</button></div>{changes.length?<div className="m-changes-list">{changes.map(c=><button className="m-change" key={c.category} onClick={()=>movement({cat:c.category,category:c.category,type:'expense',dateFrom:c.currentStart,dateTo:c.currentEnd})}><span><strong>{c.category.replace(/^[^\p{L}\p{N}]+/u,'')}</strong><small>{c.currentStart}–{c.currentEnd} · anterior {c.previousStart}–{c.previousEnd}</small></span><span className="m-change-value">{c.delta>0?'+':''}{fmt(c.delta)}<small>{c.percent==null?'Sin base para porcentaje':`${c.percent>0?'+':''}${c.percent.toFixed(0)}%`}</small></span><span aria-hidden="true">↗</span></button>)}</div>:<p className="m-empty-inline">Todavía no hay dos períodos comparables con gastos registrados.</p>}</section>
    <section className="m-recent"><div className="m-panel-heading"><h2>Últimos movimientos</h2><button className="m-text-button" onClick={()=>movement()}>Ver todos ↗</button></div><div className="m-ledger">{rows.length?rows.map(t=><button key={t.id} className="m-ledger-row" onClick={()=>movement()}><span><strong>{t.description}</strong><small>{t.date}</small></span><span className="m-ledger-category">{(t.category||'Otros').replace(/^[^\p{L}\p{N}]+/u,'')}</span><strong>{t.type==='income'?'+':t.type==='transfer'?'↔':'−'} {fmt(t.amount)}</strong></button>):<p className="m-empty-inline">Sin movimientos en este período.</p>}</div></section>
