...
@media(max-width:480px){.inp{font-size:16px!important}.kpi-grid{grid-template-columns:repeat(2,minmax(0,1fr))!important}.kpi-grid>:last-child:nth-child(odd){grid-column:1/-1}.modal{padding:15px}.ov{padding-top:max(10px,env(safe-area-inset-top));padding-right:max(10px,env(safe-area-inset-right));padding-bottom:max(10px,env(safe-area-inset-bottom));padding-left:max(10px,env(safe-area-inset-left))}.holding-metrics{grid-template-columns:1fr}.preview-actions{display:grid!important;grid-template-columns:1fr;width:100%}.preview-actions .btn{width:100%}.onboarding-panel{padding:18px}.onboarding-result{margin-bottom:16px!important}.onboarding-allocation{gap:4px!important}.onboarding-allocation>div{min-width:0!important;padding:8px 6px!important;flex:1 1 0}.onboarding-nav{margin-top:20px!important}.tour-guide{left:10px!important;right:10px!important;padding:10px 11px!important}.tour-guide-copy{max-height:82px}.score-axis-grid{grid-template-columns:repeat(2,minmax(0,1fr))!important}.tx-actions{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr));width:100%}.tx-actions .btn{width:100%}.tx-filters{display:grid!important;grid-template-columns:1fr 1fr}.tx-filters>select{width:100%!important;min-width:0!important}.tx-filters>select:nth-of-type(3){grid-column:1/-1}.tx-desktop-table,.preview-desktop{display:none!important}.tx-mobile-list,.preview-mobile{display:flex;flex-direction:column;gap:9px}.transfer-actions{display:grid!important;grid-template-columns:1fr!important;width:100%}.transfer-actions .btn{width:100%;white-space:normal}.goal-actions:has([data-tour-target="vincular-inv-btn"]){grid-template-columns:minmax(0,1fr) minmax(0,1fr) 42px!important}.goal-actions .btn{min-width:0;padding-inline:7px}.goal-input-actions{display:grid!important;grid-template-columns:minmax(0,1fr) 68px 38px 38px;gap:5px!important}.goal-input-actions .inp{width:100%!important;min-width:0}.goal-input-actions .btn{width:38px;padding:0;justify-content:center}.holding-head{flex-wrap:wrap;align-items:flex-start!important}.holding-copy{min-width:0;overflow-wrap:anywhere}.holding-actions{width:100%;justify-content:flex-end}.holding-link{flex-direction:column;align-items:stretch!important}.holding-link .inp{width:100%!important;min-width:0}.empty-panel-actions{width:100%}.empty-panel-actions .btn{flex:1;min-width:120px}.imgdrop,.dz{padding:24px 18px}.imgdrop>div:last-of-type,.dz>div:last-of-type{overflow-wrap:anywhere}.salary-form{display:grid!important;grid-template-columns:1fr!important}.salary-form>*{min-width:0!important}.salary-form .btn{width:100%}}
...
function Onboarding({update,notify}){
  ...
  {step===5&&<div className="onboarding-result" style={{textAlign:"center",marginBottom:24}}><div style={{width:72,height:72,borderRadius:22,background:`${pf.color}18`,border:`1px solid ${pf.color}35`,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 20px"}}>{pf.emoji}</div><div style={{fontSize:24,fontWeight:800,letterSpacing:"-.5px",color:pf.color}}>{pf.label}</div><div style={{fontSize:13,color:T.muted,marginTop:6,lineHeight:1.6}}>{pf.desc}</div><div className="onboarding-allocation" style={{display:"flex",justifyContent:"center",gap:6,marginTop:16}}>{[{l:"Renta fija",p:pf.pct[0],c:"#4D9EFF"},{l:"Mixto",p:pf.pct[1],c:"#FFB830"},{l:"Renta variable",p:pf.pct[2],c:"#FF4D6A"}].map(s=><div key={s.l} style={{background:T.raised,borderRadius:10,padding:"10px 14px",textAlign:"center",minWidth:80}}><div className="mono" style={{fontSize:18,fontWeight:700,color:s.c}}>{s.p}%</div><div style={{fontSize:10,color:T.muted,marginTop:3}}>{s.l}</div></div>)}</div><div className="mono" style={{fontSize:11,color:T.muted,marginTop:12}}>Score: {score}/12 · Horizonte: {horizon}</div></div>}
  <div className="onboarding-nav" style={{display:"flex",gap:10,marginTop:28}}>{step>0&&<button className="btn bg" style={{flex:.4,justifyContent:"center"}} onClick={()=>setStep(s=>s-1)}>← Atrás</button>}<button onClick={step<5?()=>setStep(s=>s+1):finish} disabled={!canNext} style={{flex:1,justifyContent:"center",display:"flex",alignItems:"center",gap:7,padding:"11px 20px",borderRadius:12,background:!canNext?"#2A2025":"linear-gradient(135deg,var(--ac),var(--acd))",color:"#09080A",fontWeight:700,fontSize:14,border:"none",cursor:canNext?"pointer":"not-allowed",transition:"all .18s",opacity:canNext?1:.45}}>{step===5?"¡Empezar! →":"Continuar →"}</button></div></div></div>);
}
    ...
    <div className="trend-grid" style={{display:"grid",gridTemplateColumns:"1fr",gap:14,marginBottom:14}}>
      ...
      <div data-tour-target="score-card" className="card up d3" style={{gridColumn:"1/-1"}}>{(()=>{
        ...
        return(<div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:6,flexShrink:0,width:isMobile?"100%":undefined,minWidth:0}}>
            ...
            <svg width={isMobile?"100%":160} height={isMobile?196:155} viewBox={isMobile?"-60 -12 280 182":"-20 -10 180 175"} style={{overflow:isMobile?"hidden":"visible",maxWidth:isMobile?300:undefined,display:"block"}}>
              ...
              {axes.map((ax,i)=>{const p=pt(i,R+14);const anchor=p.x<cx-5?"end":p.x>cx+5?"start":"middle";const isWorst=ax.key===worstItem.key;return<text key={i} x={p.x.toFixed(1)} y={p.y.toFixed(1)} textAnchor={anchor} fill={isWorst?T.coral:T.muted} fontSize={isMobile?"8.5":"7.5"} fontFamily="Sora" fontWeight={isWorst?"700":"400"}>{ax.l}</text>;})}
            </svg>
            <div style={{fontSize:11,color:sc,fontWeight:600}}>{score>=70?"Buena salud":score>=45?"En camino":"Necesita atención"}</div>
          </div>
          <div style={{flex:1,minWidth:isMobile?0:200,width:isMobile?"100%":undefined}}>
            <div className="score-axis-grid" style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:7,marginBottom:10}}>
              {axes.map((it)=>{const pct=it.pct;const barCol=pct>=0.8?"var(--ac)":pct>=0.4?T.amber:T.coral;const isWorst=it.key===worstItem.key;return(<div key={it.key} style={{background:isWorst?"rgba(255,95,109,.06)":T.raised,borderRadius:10,padding:"9px 11px",border:`1px solid ${isWorst?"rgba(255,95,109,.2)":T.border}`,minWidth:0}}><div style={{fontSize:10,fontWeight:700,color:isWorst?T.coral:T.white,marginBottom:2}}>{it.l}</div><div style={{fontSize:9,color:T.muted,marginBottom:6,lineHeight:1.4,overflowWrap:"anywhere"}}>{it.desc}</div><div style={{display:"flex",alignItems:"center",gap:6}}><div className="prog" style={{flex:1,height:4}}><div className="progf" style={{width:`${pct*100}%`,background:barCol}}/></div><span className="mono" style={{fontSize:9,color:barCol,flexShrink:0}}>{it.v}/{it.m}</span></div></div>);})}
            </div>
...
function Transactions({state,update,notify,setView}){
  ...
  const editMovement=t=>{setForm({date:t.date,description:t.description.replace("🔁 ",""),amount:t.amount,type:t.type,category:t.category,currency:"ARS",isRecurring:false});setETx(t.id);setSA(true);};
  ...
  return(<div className="up">
    ...
    <div className="tx-mobile-list">{rows.map(t=>(<div key={t.id} className="card csm" style={{opacity:t.type==="transfer"?.65:1}}><div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12}}><div style={{minWidth:0,flex:1}}><div style={{fontSize:13,fontWeight:700,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.description}</div><div style={{display:"flex",alignItems:"center",gap:7,marginTop:5,flexWrap:"wrap"}}><span className="mono" style={{fontSize:10,color:T.muted}}>{t.date}</span><span className={`tag ${t.type==="income"?"ti":t.type==="transfer"?"tt":t.category==="💰 Ahorro"?"ts":"te"}`} style={{fontSize:9,padding:"2px 7px"}}>{t.type==="income"?"Ingreso":t.type==="transfer"?"Transferencia":t.category==="💰 Ahorro"?"Ahorro":"Gasto"}</span></div></div><div className="mono" style={{fontSize:13,fontWeight:700,color:t.type==="income"?T.teal:t.type==="transfer"?T.muted:T.red,flexShrink:0}}>{t.type==="income"?"+":t.type==="transfer"?"↔":"-"}{fmt(t.amount)}</div></div><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,marginTop:11,paddingTop:10,borderTop:`1px solid ${T.border}`}}><button onClick={()=>t.type!=="transfer"&&setEC({id:t.id,cat:t.category})} style={{background:T.raised,border:`1px solid ${T.border}`,borderRadius:8,padding:"6px 9px",fontSize:10,color:T.mid,cursor:t.type==="transfer"?"default":"pointer",minWidth:0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.type==="transfer"?"↔ Transferencia interna":categoryDisplay(t.category)}</button><div style={{display:"flex",gap:6,flexShrink:0}}>{t.type!=="transfer"&&<button className="btn bg bsm" aria-label="Editar movimiento" style={{width:40,height:40,padding:0}} onClick={()=>editMovement(t)}><ic.Edit/></button>}<button className="btn bd bsm" aria-label="Eliminar movimiento" style={{width:40,height:40,padding:0}} onClick={()=>removeMovement(t.id)}><ic.Trash/></button></div></div></div>))}</div>
            ...
            {pct>=100
              ...
              :addTo?.id===g.id
                ?<div style={{display:"flex",flexDirection:"column",gap:7}}>
                    ...
                    <div className="goal-input-actions" style={{display:"flex",gap:7}}><input className="inp" style={{flex:1,fontSize:12}} placeholder="Monto" value={addAmt} onChange={e=>setAA(e.target.value)} autoFocus onKeyDown={e=>e.key==="Enter"&&addSav(g)}/><select className="inp" style={{width:70,fontSize:12,padding:"10px 6px"}} value={addCur} onChange={e=>setAC(e.target.value)}><option>ARS</option><option>USD</option></select><button className="btn bl bsm" onClick={()=>addSav(g)}>+</button><button className="btn bg bsm" onClick={()=>{setAT(null);setAA("");setAL("");}}>✕</button></div>
                  </div>
