...
function Goals({state,update,notify}){
  ...
  {linkGoal&&<div className="ov" onClick={e=>e.target===e.currentTarget&&setLG(null)}><div className="modal" style={{width:420}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}><h2 style={{fontSize:17,fontWeight:700}}>🔗 Vincular inversión a meta</h2><button className="btn bg bsm" onClick={()=>setLG(null)}><ic.X/></button></div><div style={{fontSize:12,color:T.mid,marginBottom:14}}>Seleccioná una inversión para asociarla a esta meta. Su valor de mercado contará hacia el progreso.</div>{holdings.filter(h=>!h.goalId).length===0?<div style={{color:T.muted,fontSize:12,textAlign:"center",padding:"20px 0"}}>No hay inversiones disponibles (todas ya vinculadas a alguna meta)</div>:holdings.filter(h=>!h.goalId).map(h=>{const {curArs}=calcHoldingValueArs(h,state.marketPrices,state.usdRate);return<div key={h.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:T.raised,borderRadius:10,padding:"12px 14px",marginBottom:8,border:`1px solid ${T.border}`}}><div><div style={{fontSize:13,fontWeight:600}}>{h.ticker||h.name} <span style={{fontSize:10,color:T.muted}}>{h.type}</span></div><div className="mono" style={{fontSize:11,color:T.blue,marginTop:2}}>{fmt(curArs)}</div></div><button className="btn bl bsm" onClick={()=>{update({holdings:holdings.map(x=>x.id===h.id?{...x,goalId:linkGoal}:x)});setLG(null);notify("Inversión vinculada ✓");}}>Vincular</button></div>;})}}</div></div>}
