...
const T = {
  ...
  white:"#F2EBE0",mid:"#8A7D75",muted:"#48403A",ink:"#09080A",red:"#FF5F6D",
};
const SK="fp_v3b";
...
const cleanJSON=r=>{if(!r)return"";let s=r.replace(/```json|```/gi,"").trim();const f=s.search(/[\{\[]/);const l=Math.max(s.lastIndexOf("}"),s.lastIndexOf("]"));return f!==-1&&l!==-1?s.slice(f,l+1):s;};
...
async function ai(prompt,sys=""){try{const r=await fetch(AI_URL,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-5-20250929",max_tokens:1500,system:sys||"Respond with clean JSON only. No markdown fences.",messages:[{role:"user",content:prompt}]})});if(!r.ok)throw new Error(`HTTP ${r.status}`);const d=await r.json();return(d.content?.filter(b=>b.type==="text").map(b=>b.text).join("")||"").replace(/```json|```/g,"").trim();}catch(e){console.error("AI error",e);return null;}}
async function aiVision(b64,rawMime,prompt){const VALID=["image/jpeg","image/png","image/gif","image/webp"];const mime=VALID.includes(rawMime)?rawMime:"image/png";try{const r=await fetch(AI_URL,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-5-20250929",max_tokens:1500,system:"Financial data extractor. Return ONLY valid JSON, no markdown.",messages:[{role:"user",content:[{type:"image",source:{type:"base64",media_type:mime,data:b64}},{type:"text",text:prompt}]}]})});if(!r.ok){const e=await r.text();throw new Error(`HTTP ${r.status}: ${e}`);}const d=await r.json();return(d.content?.filter(b=>b.type==="text").map(b=>b.text).join("")||"").replace(/```json|```/g,"").trim();}catch(e){console.error("Vision error",e);return null;}}
async function extractFromImage(b64,mime){const CATS_STR=["🏠 Vivienda","🛒 Supermercado","🚗 Transporte","🍔 Comida y delivery","💊 Salud","👕 Indumentaria","📱 Servicios digitales","🎬 Ocio","💪 Deporte","✈️ Viajes","📚 Educación","💰 Ahorro","💳 Cuotas","🐜 Gastos hormiga","🧛 Suscripciones","❓ Otros"].join(", ");const raw=await aiVision(b64,mime,`Extract ALL transactions from this Argentine bank/wallet/credit card screenshot. Return ONLY JSON: {"transactions":[{"date":"YYYY-MM-DD","description":"<desc>","amount":<positive number>,"type":"income|expense","category":"<one of: ${CATS_STR}>" }],"currency":"ARS|USD","appDetected":"<app or null>"}\nRules:\n- DATE: exact date. Unknown use today.\n- AMOUNTS: always positive.\n- Skip scientific notation rows.\n- category MUST include emoji prefix.`);if(!raw)return null;try{const parsed=JSON.parse(cleanJSON(raw));if(parsed.transactions){parsed.transactions=parsed.transactions.map(t=>({...t,category:matchCat(t.category)}));}return parsed;}catch{return null;}}
async function loadPDFJS(){if(window.pdfjsLib)return window.pdfjsLib;return new Promise(resolve=>{const s=document.createElement("script");s.src="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.min.js";s.onload=()=>{window.pdfjsLib.GlobalWorkerOptions.workerSrc="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js";resolve(window.pdfjsLib);};document.head.appendChild(s);});}
async function extractFromPDF(file,onProgress){const lib=await loadPDFJS();const buf=await file.arrayBuffer();const pdf=await lib.getDocument({data:buf}).promise;const results=[];for(let p=1;p<=pdf.numPages;p++){onProgress?.(`Procesando página ${p}/${pdf.numPages}...`);const page=await pdf.getPage(p);const vp=page.getViewport({scale:2.0});const canvas=document.createElement("canvas");canvas.width=vp.width;canvas.height=vp.height;const ctx=canvas.getContext("2d");await page.render({canvasContext:ctx,viewport:vp}).promise;const b64=canvas.toDataURL("image/jpeg",0.85).split(",")[1];const r=await extractFromImage(b64,"image/jpeg");if(r?.transactions?.length){results.push(...r.transactions.map((t,i)=>({...t,id:`pdf_${uid()}_${p}_${i}`,currency:r.currency||"ARS",source:"pdf"})));}}return results;}
...
async function autoCat(desc){const CATS_STR=["🏠 Vivienda","🛒 Supermercado","🚗 Transporte","🍔 Comida y delivery","💊 Salud","👕 Indumentaria","📱 Servicios digitales","🎬 Ocio","💪 Deporte","✈️ Viajes","📚 Educación","💰 Ahorro","💳 Cuotas","🐜 Gastos hormiga","🧛 Suscripciones","❓ Otros"].join(", ");const raw=await ai(`Transaction: "${desc}". Return ONLY: {"category":"<one of: ${CATS_STR}>","type":"income|expense"} Rules: sueldo/salary→income; netflix/spotify→🧛 Suscripciones; rappi/pedidosya→🍔 Comida y delivery; gym→💪 Deporte; coto/carrefour→🛒 Supermercado; cuota→💳 Cuotas. category MUST include emoji prefix.`);if(!raw)return{category:"❓ Otros",type:"expense"};try{const p=JSON.parse(cleanJSON(raw));return{category:matchCat(p.category),type:p.type||"expense"};}catch{return{category:"❓ Otros",type:"expense"};}}
async function genWeeklyInsight(transactions,usdRate,portfolioValueArs=0,portfolioInvestedArs=0,holdings=[]){const now=Date.now();const w1=transactions.filter(t=>(now-new Date(t.date))<=7*864e5);const e1=w1.filter(t=>t.type==="expense").reduce((s,t)=>s+t.amount,0);const w2=transactions.filter(t=>{const d=now-new Date(t.date);return d>7*864e5&&d<=14*864e5;});const e2=w2.filter(t=>t.type==="expense").reduce((s,t)=>s+t.amount,0);const cm={};w1.filter(t=>t.type==="expense").forEach(t=>{cm[t.category]=(cm[t.category]||0)+t.amount;});const topCat=Object.entries(cm).sort((a,b)=>b[1]-a[1])[0];const top3=Object.entries(cm).sort((a,b)=>b[1]-a[1]).slice(0,3).map(([c,v])=>`${c}:${fARS(v)}`).join(", ");const pnlArs=portfolioValueArs-portfolioInvestedArs;const pnlPct=portfolioInvestedArs>0?((pnlArs/portfolioInvestedArs)*100).toFixed(1):"0";const holdingsSummary=holdings.slice(0,3).map(h=>`${h.ticker||h.name}`).join(", ");const raw=await ai(`Argentina personal finance coach. Data: last7d_expenses=${fARS(e1)}, prior_week=${fARS(e2)}, top_categories=[${top3}], top_cat_name="${topCat?topCat[0]:"N/A"}", top_cat_amount=${fARS(topCat?topCat[1]:0)}, portfolio_value=${fARS(portfolioValueArs)}, portfolio_invested=${fARS(portfolioInvestedArs)}, portfolio_pnl=${fARS(pnlArs)}, portfolio_pnl_pct=${pnlPct}%, holdings="${holdingsSummary}", usd_rate=${usdRate}, date=${todayISO()}.\nGenerate EXACTLY 4 insight cards in Spanish. Return ONLY valid JSON:\n{"cards":[{"type":"spending","icon":"💸","title":"<5 words>","value":"<amount or %>","detail":"<1 motivating sentence>","trend":"up|down|stable","color":"red|amber|lime"},{"type":"top_category","icon":"<emoji>","title":"<5 words>","value":"<amount>","detail":"<1 tip>","trend":"up|down|stable","color":"amber|red|lime"},{"type":"investment","icon":"📊","title":"<5 words>","value":"<pnl%>","detail":"<1 sentence>","trend":"up|down|stable","color":"lime|blue|red"},{"type":"goal","icon":"🎯","title":"<5 words>","value":"<% or metric>","detail":"<1 sentence>","trend":"up|down|stable","color":"blue|lime|amber"}],"headline":"<10 words max Spanish>"}`, "Friendly Argentine financial coach. Respond ONLY with valid JSON.");if(!raw)return null;try{return JSON.parse(cleanJSON(raw));}catch{return null;}}
...
function parseCSV(txt){const lines=txt.trim().split("\n").filter(l=>l.trim());if(lines.length<2)return[];const sep=lines[0].includes(";")?";":(lines[0].includes("\t")?"\t":",");let headerIdx=0;for(let i=0;i<Math.min(lines.length,10);i++){const cols=parseCSVLine(lines[i],sep).map(c=>c.toLowerCase().replace(/"/g,""));const hasDate=cols.some(c=>/^(fecha|date|release_date|transaction_date|dia)/.test(c));const hasAmt=cols.some(c=>/^(transaction_amount|monto|importe|debito|credito|amount|debit|credit)/.test(c));if(hasDate||hasAmt){headerIdx=i;break;}}const hdrs=parseCSVLine(lines[headerIdx],sep).map(h=>h.trim().replace(/"/g,"").toLowerCase());const out=[];for(let i=headerIdx+1;i<lines.length;i++){const cols=parseCSVLine(lines[i],sep);const obj={};hdrs.forEach((h,idx)=>obj[h]=(cols[idx]||"").trim().replace(/^"|"$/g,""));const dK=hdrs.find(h=>!h.includes("reference")&&(/^(fecha|date|release_date|transaction_date|dia)$/.test(h)||/fecha|date/.test(h)));const dscK=hdrs.find(h=>!h.includes("reference")&&(/^(description|descripcion|concepto|detalle|establecimiento|comercio|transaction_type)$/.test(h)||/desc|concepto|detalle|comer|estab/.test(h)));const debK=hdrs.find(h=>/^(debito|debe|debit|cargo|egreso)$/.test(h));const creK=hdrs.find(h=>/^(credito|haber|credit|abono)$/.test(h));const aK=hdrs.find(h=>/^(transaction_amount|importe|monto|amount|total)$/.test(h)||(/import|monto|amount|total/.test(h)&&!h.includes("reference")&&!h.includes("balance")));let amount=0,type="expense";if(debK&&creK){const deb=px(obj[debK]),cre=px(obj[creK]);if(cre>0){amount=cre;type="income";}else if(deb>0){amount=deb;type="expense";}else continue;}else if(aK){const rawStr=String(obj[aK]||"").trim();if(/e[+\-]/i.test(rawStr))continue;amount=Math.abs(px(obj[aK]));type=rawStr.startsWith("-")||px(obj[aK])<0?"expense":"income";}else{const numVal=Object.entries(obj).filter(([k])=>!k.includes("reference")&&!k.includes("balance")).map(([,v])=>px(v)).find(v=>v>0&&v<1e10);if(!numVal)continue;amount=numVal;type="expense";}if(amount<=0||amount>1e10)continue;const rawDate=(obj[dK]||"").trim();out.push({id:`csv_${uid()}_${i}`,currency:"ARS",date:rawDate||todayISO(),description:obj[dscK]||`TX ${i-headerIdx}`,amount,type,category:"❓ Otros"});}return out;}
...
const healthScore=(txs,goals,holdings=[],salaries=[])=>{const CUR=getCUR();const c=txs.filter(t=>gMonth(t.date)===CUR);const inc=c.filter(t=>t.type==="income").reduce((s,t)=>s+t.amount,0)+getSalaryTotal(salaries,CUR);const exp=c.filter(t=>t.type==="expense").reduce((s,t)=>s+t.amount,0);const cuotas=c.filter(t=>t.category==="💳 Cuotas").reduce((s,t)=>s+t.amount,0);const hormiga=c.filter(t=>t.category==="🐜 Gastos hormiga").reduce((s,t)=>s+t.amount,0);const vampiro=c.filter(t=>t.category==="🧛 Suscripciones").reduce((s,t)=>s+t.amount,0);const prev3=[-1,-2,-3].map(offset=>{const d=new Date();d.setMonth(d.getMonth()+offset);const m=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`;return txs.filter(t=>gMonth(t.date)===m&&t.category==="🧛 Suscripciones").reduce((s,t)=>s+t.amount,0);});const avgVampiro=prev3.reduce((s,v)=>s+v,0)/3;const vampiroGrowing=avgVampiro>0&&vampiro>avgVampiro*1.10;const positiveMonths=[-1,-2,-3].reduce((count,offset)=>{const d=new Date();d.setMonth(d.getMonth()+offset);const m=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`;const mInc=txs.filter(t=>gMonth(t.date)===m&&t.type==="income").reduce((s,t)=>s+t.amount,0)+getSalaryTotal(salaries,m);const mExp=txs.filter(t=>gMonth(t.date)===m&&t.type==="expense").reduce((s,t)=>s+t.amount,0);return count+(mInc>mExp?1:0);},0);const activeGoals=goals.filter(g=>{const cash=(g.saved||0)+(g.payments||g.milestones||[]).reduce((s,p)=>s+p.amount,0);return cash<g.target;});const goalProgress=activeGoals.length>0?activeGoals.reduce((s,g)=>{const cash=(g.saved||0)+(g.payments||g.milestones||[]).reduce((a,p)=>a+p.amount,0);const inv=holdings.filter(h=>h.goalId===g.id).reduce((a,h)=>a+(h.curArs||0),0);return s+clamp((cash+inv)/g.target,0,1);},0)/activeGoals.length:0;const hasEmergencyGoal=goals.some(g=>/emergencia|colch[oó]n/i.test(g.name));const holdingTypes=new Set(holdings.map(h=>h.type||"other"));const divScore=holdingTypes.size===0?0:holdingTypes.size===1?5:holdingTypes.size>=2?10:0;const savRate=inc>0?clamp((inc-exp)/inc,0,1):0;const s1=inc>0?clamp(Math.round((savRate/0.20)*20),0,20):0;const savPct=Math.round(savRate*100);const s2=positiveMonths===3?15:positiveMonths===2?10:positiveMonths===1?5:0;const debtRate=inc>0?clamp(cuotas/inc,0,1):0;const s3=inc>0?Math.round(Math.max(0,15-(debtRate/0.30)*15)):cuotas===0?15:0;const hormigaPct=inc>0?hormiga/inc:0;const s4h=hormigaPct<0.05?8:hormigaPct<0.10?5:hormigaPct<0.15?2:0;const s4v=vampiroGrowing?0:avgVampiro===0?7:4;const s4=s4h+s4v;const s5base=activeGoals.length>0?Math.round(goalProgress*12):0;const s5=clamp(s5base+(hasEmergencyGoal?3:0),0,15);const s6=divScore;const hasGoalProgress=goalProgress>0||activeGoals.length>0;const s7=(holdings.length>0&&hasGoalProgress)?10:(holdings.length>0||hasGoalProgress)?5:0;const score=clamp(Math.round(s1+s2+s3+s4+s5+s6+s7),0,100);const tips={ahorro:savPct>=20?"Tasa de ahorro ideal — mantené la consistencia.":`Ahorrás el ${savPct}% de tu ingreso. Llegar al 20% te suma ${20-s1} pts.`,consistencia:s2===15?"3 meses consecutivos en positivo. Excelente.":`Cerraste ${positiveMonths}/3 meses en positivo. Cada mes en verde suma 5 pts.`,deuda:debtRate<0.10?"Deuda bajo control.":`Cuotas = ${Math.round(debtRate*100)}% del ingreso. El límite saludable es 30%.`,fugas:(()=>{const parts=[];if(hormigaPct>=0.05)parts.push(`Gastos hormiga = ${Math.round(hormigaPct*100)}% del ingreso`);if(vampiroGrowing)parts.push(`Suscripciones subieron ${Math.round((vampiro/avgVampiro-1)*100)}% vs últimos 3 meses`);return parts.length>0?parts.join(" · ")+"  → revisá Recurrentes.":"Fugas invisibles bajo control.";})(),metas:activeGoals.length===0?"Sin metas activas — crear una suma hasta 12 pts.":goalProgress>0.5?"Metas avanzando bien.":`Progreso promedio de metas: ${Math.round(goalProgress*100)}%. Cada depósito suma.`,diversif:divScore===0?"Sin inversiones — agregar cualquier instrumento suma 5 pts.":divScore<10?"Invertís en 1 tipo de activo. Diversificar suma hasta 5 pts más.":"Portfolio diversificado en 2+ tipos de activos.",perfil:s7===10?"Inversiones y metas activas — perfil completo.":s7===5?"Tenés inversiones o metas pero no ambas — completar el combo suma 5 pts.":"Sin inversiones ni metas activas. Empezar con cualquiera suma puntos.",};return{score,tips,items:[{key:"ahorro",l:"Tasa de ahorro",desc:"Cuánto guardás de cada peso que ganás",v:s1,m:20,tip:tips.ahorro},{key:"consistencia",l:"Consistencia",desc:"Meses consecutivos cerrando en positivo",v:s2,m:15,tip:tips.consistencia},{key:"deuda",l:"Control de deuda",desc:"Cuotas como porcentaje del ingreso",v:s3,m:15,tip:tips.deuda},{key:"fugas",l:"Fugas invisibles",desc:"Gastos hormiga y suscripciones sin usar",v:s4,m:15,tip:tips.fugas},{key:"metas",l:"Avance en metas",desc:"Progreso real hacia tus objetivos activos",v:s5,m:15,tip:tips.metas},{key:"diversif",l:"Diversificación",desc:"Variedad de instrumentos en tu portfolio",v:s6,m:10,tip:tips.diversif},{key:"perfil",l:"Perfil completo",desc:"Tenés inversiones y metas activas",v:s7,m:10,tip:tips.perfil},]};};
...
html,body{background:#09080A;color:#F2EBE0;font-family:'Sora',sans-serif;overflow:hidden;height:100%;transition:background .6s ease,color .4s ease}
body.usd-mode{background:#030810}
body.usd-mode .card{border-color:#152030;background:rgba(9,18,32,.9);transition:border-color .3s,box-shadow .3s,background .6s}
...
body.usd-mode h1,body.usd-mode h2{color:#D8EEF0;transition:color .4s}
...
.bl{background:linear-gradient(135deg,var(--ac),var(--acd));color:#09080A}
...
.ti{background:rgba(0,212,170,.1);color:#00D4AA}.te{background:rgba(255,95,109,.1);color:#FF5F6D}.ts{background:rgba(91,158,255,.1);color:#5B9EFF}.tt{background:rgba(72,64,58,.15);color:#8A7D75}
...
.tab.on{background:#100E12;color:var(--ac);box-shadow:0 0 0 1px rgba(var(--ac-rgb),.35)}
...
@media(max-width:768px){.hide-m{display:none!important}.modal{padding:18px;width:100%;border-radius:18px;max-height:92vh}.card{padding:14px}.kpi-grid{grid-template-columns:1fr 1fr!important}.trend-grid{grid-template-columns:1fr!important}.g2,.g3{grid-template-columns:1fr!important}.tbl td,.tbl th{padding:7px 9px;font-size:11px}.inv-grid{grid-template-columns:1fr!important}.tabbar{gap:2px}.tab{padding:6px 10px;font-size:11px}.btn{padding:9px 14px;font-size:12px}}
@media(max-width:480px){.kpi-grid{grid-template-columns:1fr!important}.modal{padding:14px}}`;
...
const ic={
  ...
  Target:()=><svg width="16" height="16" viewBox="0 0 20 20" fill="none"><path d="M10 3C7 3 5 5.5 5 8.5c0 4 3.5 6.5 5 8 1.5-1.5 5-4 5-8C15 5.5 13 3 10 3z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round"/><circle cx="10" cy="8" r="1.5" fill="currentColor" opacity=".6"/></svg>,
...
function TourGuide({setView,scoreTips={}}){
  ...
  return(<div style={{...tooltipStyle,background:"rgba(16,14,18,.97)",border:`1px solid ${ac}`,borderRadius:16,padding:"13px 16px",zIndex:500,backdropFilter:"blur(20px)",boxShadow:`0 12px 48px rgba(0,0,0,.75)`,display:"flex",alignItems:"center",gap:11,animation:"up .3s ease",position:"fixed"}}>{arrowStyle&&<div style={arrowStyle}/>}<div style={{flex:1,minWidth:0}}><div style={{fontSize:9,fontWeight:800,marginBottom:5,textTransform:"uppercase",letterSpacing:"1px",display:"flex",alignItems:"center",gap:6,color:acText}}><span style={{width:6,height:6,borderRadius:"50%",background:acText,display:"inline-block",animation:isCharla?"pulse-glow 1.2s infinite":"none",boxShadow:`0 0 8px ${acText}`}}/>{isCharla?"Charla en vivo":"Guía Mangos"}</div><div style={{fontSize:12,color:"#E8DDD4",lineHeight:1.6,fontWeight:500}}>{tip.tip}</div>{(()=>{const key=tip.scoreTipKey;if(!key||!scoreTips)return null;const stip=scoreTips[key];if(!stip)return null;return<div style={{fontSize:10,color:T.mango,marginTop:5,paddingTop:5,borderTop:"1px solid rgba(255,154,53,.15)",lineHeight:1.5}}><span style={{fontWeight:700}}>Tu situación: </span>{stip}</div>;})()}</div><div style={{display:"flex",flexDirection:"column",gap:5,flexShrink:0,alignItems:"stretch"}}><button onClick={()=>{setView(tip.section);setTimeout(()=>{const targetId=tip.target||tip.section;const p=getAnchor(targetId);setPos(p);if(tip.target)triggerTarget(tip.target,tip.section,tip.fallbackTarget,tip.fallbackSection);},180);}} style={{background:`linear-gradient(135deg,var(--ac),var(--acd))`,color:"#09080A",padding:"6px 12px",borderRadius:"8px",fontSize:"11px",fontWeight:"700",border:"none",cursor:"pointer",whiteSpace:"nowrap"}}>{tip.btn}</button><div style={{display:"flex",gap:4}}>{!isCharla&&<button className="btn bg bsm" onClick={next} style={{flex:1,justifyContent:"center",fontSize:11,padding:"4px 8px"}}>→</button>}<button className="btn bg bsm" style={{flex:1,justifyContent:"center",padding:"4px 8px",fontSize:11}} onClick={dismiss}>✕</button></div></div></div>);
}
...
export default function App(){
  ...
  useEffect(()=>{if(!ready)return;const hasBadDates=state.transactions?.some(t=>t.date.includes("/")||(t.date.includes("-")&&t.date.indexOf("-")!==4));if(hasBadDates){const fixed=state.transactions.map(t=>{if(t.date.includes("/")||(t.date.includes("-")&&t.date.indexOf("-")!==4)){const parts=t.date.split(/[\/\-]/);if(parts.length>=3){const[d,m,y]=parts[0].length===4?[parts[2],parts[1],parts[0]]:[parts[0],parts[1],parts[2]];const year=y.length===2?"20"+y:y;return{...t,date:`${year}-${m.padStart(2,"0")}-${d.padStart(2,"0")}`};}}return t;});setState(s=>({...s,transactions:fixed}));}},[ ready, state.transactions]);
  ...
  const pages={dashboard:<Dashboard state={state} update={update} notify={notify} setView={navTo}/>,transactions:<Transactions state={state} update={update} notify={notify}/>,goals:<Goals state={state} update={update} notify={notify}/>,salary:<SalaryModule state={state} update={update} notify={notify}/>,analytics:<Analytics state={state} update={update} setView={navTo}/>,investments:<Investments state={state} update={update} notify={notify}/>,import:<Import state={state} update={update} notify={notify}/>};
  ...
  <div style={{background:T.raised,border:`1px solid ${T.border}`,borderRadius:14,padding:"14px"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
      ...
      {state.displayCurrency==="USD"&&<span style={{fontSize:9,fontWeight:800,padding:"2px 7px",borderRadius:99,background:"rgba(91,207,184,.15)",color:"#5BCFB8",border:"1px solid rgba(91,207,184,.25)",letterSpacing:.5}}>ACTIVO</span>}
    ...
    <div className="mono" style={{fontSize:24,fontWeight:600,color:currAccent.accent,textAlign:"center",transition:"color .4s"}}>{fARS(state.usdRate)}</div>
    <div style={{display:"flex",gap:6,marginTop:14}}>
      {["ARS","USD"].map(c=>(<button key={c} data-tour-target={c==="USD"?"toggle-usd":undefined} onClick={()=>update({displayCurrency:c})} style={{flex:1,padding:"6px 0",borderRadius:8,fontSize:10,fontWeight:600,border:`1px solid ${state.displayCurrency===c?(c==="USD"?"rgba(91,207,184,.4)":"rgba(204,255,71,.35)"):T.border}`,background:state.displayCurrency===c?(c==="USD"?"rgba(91,207,184,.1)":"rgba(204,255,71,.08)"):T.surface,color:state.displayCurrency===c?(c==="USD"?"#5BCFB8":T.lime):T.muted,cursor:"pointer",transition:"all .3s"}}>{c}</button>))}
    </div>
  </div>
  <div style={{fontSize:9,color:T.muted,textAlign:"center",marginTop:16,lineHeight:1.4,padding:"0 10px"}}>⚠️ Mangos es una herramienta educativa y de gestión personal. No constituye asesoramiento financiero.</div>
  ...
  return(<div style={{display:"flex",height:"100vh",overflow:"hidden",background:T.bg}}><style>{CSS}</style>{state.displayCurrency==="USD"&&<USDAtmosphere/>}{isMobile?<>{sideOpen&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:299}} onClick={()=>setSO(false)}/>}{sidebar}</>:sidebar}{isMobile&&<div style={{position:"fixed",top:0,left:0,right:0,height:52,background:T.surface,borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",padding:"0 14px",gap:12,zIndex:100}}><button onClick={()=>setSO(true)} style={{color:T.white,padding:4}}><ic.Menu/></button><div style={{fontSize:14,fontWeight:800,letterSpacing:"-.4px",background:"linear-gradient(135deg,#F2EBE0,#FF9A35)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Mangos</div><div style={{flex:1}}/><div className="mono" style={{fontSize:11,color:"var(--ac)"}}>{state.displayCurrency==="USD"?fUSD(1):fARS(state.usdRate)}</div></div>}<main style={{flex:1,overflow:"auto",padding:isMobile?"66px 14px 20px":"28px 32px",transition:"background .6s",position:"relative",zIndex:2}}>{pages[view]}</main>{toast&&<div className={`toast t${toast.type}`}>{toast.msg}</div>}<TourGuide setView={navTo} scoreTips={healthScore(state.transactions||[],state.goals||[],state.holdings||[],state.salaries||[]).tips}/></div>);
}

function Onboarding({update,notify}){
  const [step,setStep]=useState(0);const [d,setD]=useState({name:"",income:"",incomeCurrency:"ARS",goalName:"",goalAmt:"",goalDate:""});const [ans,setAns]=useState([null,null,null,null]);const STEPS=6;const pickAns=(qi,val)=>{const a=[...ans];a[qi]=val;setAns(a);};const score=ans.reduce((s,v)=>s+(v||0),0);const profile=score<=3?"conservador":score<=7?"moderado":"agresivo";const horizon=ans[3]===0?"3m":ans[3]===1?"3m":ans[3]===2?"1a":"3a";
  const profileData={conservador:{emoji:<svg width="28" height="28" viewBox="0 0 20 20" fill="none"><path d="M10 3L4 6v4c0 3.5 2.5 6.5 6 7.5 3.5-1 6-4 6-7.5V6L10 3z" stroke="#5B9EFF" strokeWidth="1.5"/><path d="M7 10l2 2 4-4" stroke="#5B9EFF" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/></svg>,label:"Conservador",color:"#5B9EFF",desc:"Priorizás seguridad. Instrumentos recomendados: plazo fijo, FCI T+0, bonos CER.",pct:[70,25,5]},moderado:{emoji:<svg width="28" height="28" viewBox="0 0 20 20" fill="none"><path d="M10 3v14M5 7l5 4 5-4M5 13l5-4 5 4" stroke="#FFB830" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>,label:"Moderado",color:"#FFB830",desc:"Buscás equilibrio entre riesgo y retorno. Mix de renta fija y variable.",pct:[40,35,25]},agresivo:{emoji:<svg width="28" height="28" viewBox="0 0 20 20" fill="none"><path d="M10 2C10 2 6 8 6 13a4 4 0 008 0c0-5-4-11-4-11z" stroke="#FF5F6D" strokeWidth="1.5" strokeLinejoin="round"/></svg>,label:"Agresivo",color:"#FF5F6D",desc:"Buscás máximo crecimiento. CEDEARs, acciones, ETFs con mayor volatilidad.",pct:[15,25,60]}};
  ...
  {step===5&&<><div style={{textAlign:"center",marginBottom:24}}><div style={{width:72,height:72,borderRadius:22,background:`${pf.color}18`,border:`1px solid ${pf.color}35`,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 20px"}}>{pf.emoji}</div><div style={{fontSize:24,fontWeight:800,letterSpacing:"-.5px",color:pf.color}}>{pf.label}</div><div style={{fontSize:13,color:T.muted,marginTop:6,lineHeight:1.6}}>{pf.desc}</div><div style={{display:"flex",justifyContent:"center",gap:6,marginTop:16}}>{[{l:"Renta fija",p:pf.pct[0],c:"#4D9EFF"},{l:"Mixto",p:pf.pct[1],c:"#FFB830"},{l:"Renta variable",p:pf.pct[2],c:"#FF4D6A"}].map(s=><div key={s.l} style={{background:T.raised,borderRadius:10,padding:"10px 14px",textAlign:"center",minWidth:80}}><div className="mono" style={{fontSize:18,fontWeight:700,color:s.c}}>{s.p}%</div><div style={{fontSize:10,color:T.muted,marginTop:3}}>{s.l}</div></div>)}</div><div className="mono" style={{fontSize:11,color:T.muted,marginTop:12}}>Score: {score}/12 · Horizonte: {horizon}</div></div><div style={{background:T.raised,border:`1px solid ${T.border}`,borderRadius:12,padding:"14px 16px",textAlign:"center"}}><div style={{fontSize:14,fontWeight:700,marginBottom:4}}>¡Todo listo!</div><div style={{fontSize:12,color:T.mid,lineHeight:1.6,marginBottom:12}}>Creá tu primera meta para empezar a trackear tu progreso.</div><button className="btn bl" style={{width:"100%",justifyContent:"center"}} onClick={()=>{finish();}}>Crear mi primera meta →</button></div></>}
...
function Dashboard({state,update,notify,setView}){
  ...
  const otherInc=cur.filter(t=>t.type==="income"&&t.source!=="salary").reduce((s,t)=>s+t.amount,0);
  const inc=salary+otherInc;
  ...
  const {score,items,tips}=healthScore(transactions,goals,holdings,salaries);
  const sc=score>=70?"var(--ac)":score>=45?T.amber:T.coral;
  ...
  const trend=Array.from({length:6},(_,i)=>{const d=new Date(NOW.getFullYear(),NOW.getMonth()-5+i,1);const m=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`;const txs=transactions.filter(t=>gMonth(t.date)===m);const mSal=getSalaryTotal(salaries,m);const mOth=txs.filter(t=>t.type==="income"&&t.source!=="salary").reduce((s,t)=>s+t.amount,0);return{name:MOS[d.getMonth()],Gastos:toDsp(txs.filter(t=>t.type==="expense").reduce((s,t)=>s+t.amount,0)),Ingresos:toDsp(mSal+mOth)};});
  ...
  const kpis=[{l:"Sueldo del mes",v:salary>0?fmt(salary):"-",c:T.mango,i:<IcSalaryKpi/>,sub:salary>0?"Registrado":"Registrá tu sueldo"},{l:"Ingresos",v:fmt(inc),c:T.teal,i:<IcIncome/>,sub:"Total mensual"},{l:"Gastos",v:fmt(exp),c:T.coral,i:<IcExpense/>,sub:delta?`${delta>0?"+":""}${delta}% vs mes ant.`:"-"},{l:"Balance libre",v:fmt(inc-exp),c:(inc-exp)>=0?T.lime:T.coral,i:<IcBalance/>,sub:(inc-exp)>=0?"✓ Superávit":"⚠ Déficit"},{l:"Portfolio Real",v:fmt(portfolioValue),c:portfolioPnL>=0?T.blue:T.amber,i:<IcPortfolio/>,sub:portfolioValue>0?`P&L: ${portfolioPnL>=0?"+":""}${fmt(portfolioPnL)}`:"Sin inversiones"}];
  const refreshInsight=async()=>{if(transactions.length<3)return notify("Necesitás más transacciones","info");setLI(true);const ins=await genWeeklyInsight(transactions,usdRate,portfolioValue,portfolioInvested,holdings);if(ins){update({weeklyInsight:ins,weeklyInsightDate:todayISO()});notify("Resumen actualizado ✓");}else notify("Error — reintentá","err");setLI(false);};
  return(<div className="up">
    ...
    <div className="trend-grid" style={{display:"grid",gridTemplateColumns:"1.8fr .8fr",gap:14,marginBottom:14}}>
      <div className="card up d2"><div style={{fontSize:12,fontWeight:600,color:T.mid,marginBottom:12}}>Últimos 6 meses</div><div style={{width:"100%",minWidth:0,overflow:"hidden"}}><ResponsiveContainer width="100%" height={175}><AreaChart data={trend}><defs><linearGradient id="gi" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={T.teal} stopOpacity={.25}/><stop offset="95%" stopColor={T.teal} stopOpacity={0}/></linearGradient><linearGradient id="ge" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={T.red} stopOpacity={.25}/><stop offset="95%" stopColor={T.red} stopOpacity={0}/></linearGradient></defs><CartesianGrid stroke={T.border} strokeDasharray="3 3" vertical={false}/><XAxis dataKey="name" tick={{fill:T.muted,fontSize:10}} axisLine={false} tickLine={false}/><YAxis tick={{fill:T.muted,fontSize:9}} axisLine={false} tickLine={false} tickFormatter={v=>v>=1000?`${(v/1000).toFixed(0)}k`:v}/><Tooltip content={<CTip dc={state.displayCurrency}/>}/><Area type="monotone" dataKey="Ingresos" stroke={T.teal} fill="url(#gi)" strokeWidth={2}/><Area type="monotone" dataKey="Gastos" stroke={T.red} fill="url(#ge)" strokeWidth={2}/></AreaChart></ResponsiveContainer></div></div>
      <div data-tour-target="score-card" className="card up d3" style={{gridColumn:"1/-1"}}>{(()=>{
        const axes=items.map((it,i)=>({...it,pct:it.m>0?it.v/it.m:0}));const N=axes.length;const R=52,cx=70,cy=68;const rings=[0.33,0.67,1.0];const pt=(idx,r)=>{const a=(idx/N)*Math.PI*2-Math.PI/2;return{x:cx+r*Math.cos(a),y:cy+r*Math.sin(a)};};const gridPts=(r)=>axes.map((_,i)=>pt(i,R*r));const dataPts=axes.map((ax,i)=>pt(i,R*ax.pct));const toStr=pts=>pts.map(p=>`${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(" ");const worstItem=axes.reduce((w,it)=>it.pct<w.pct?it:w,axes[0]);
        return(<div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:6,flexShrink:0}}>
            <div style={{fontSize:10,color:T.muted,textTransform:"uppercase",letterSpacing:".8px",fontWeight:600,marginBottom:2}}>Score financiero</div>
            <svg width={160} height={155} viewBox="-20 -10 180 175" style={{overflow:"visible"}}>
              {rings.map((r,ri)=><polygon key={ri} points={toStr(gridPts(r))} fill="none" stroke={T.border} strokeWidth=".8" opacity={.6}/>)}
              {axes.map((_,i)=>{const p=pt(i,R);return<line key={i} x1={cx} y1={cy} x2={p.x.toFixed(1)} y2={p.y.toFixed(1)} stroke={T.border} strokeWidth=".8" opacity={.5}/>;})}<polygon points={toStr(dataPts)} fill="var(--ac)" fillOpacity=".15" stroke="var(--ac)" strokeWidth="1.5"/>
              {dataPts.map((p,i)=><circle key={i} cx={p.x} cy={p.y} r="2.5" fill="var(--ac)"/>)}
              <text x={cx} y={cy-5} textAnchor="middle" fill={sc} fontSize="22" fontWeight="700" fontFamily="'DM Mono',monospace">{score}</text>
              <text x={cx} y={cy+10} textAnchor="middle" fill={T.muted} fontSize="8" fontFamily="Sora">/100</text>
              {axes.map((ax,i)=>{const p=pt(i,R+14);const anchor=p.x<cx-5?"end":p.x>cx+5?"start":"middle";const isWorst=ax.key===worstItem.key;return<text key={i} x={p.x.toFixed(1)} y={p.y.toFixed(1)} textAnchor={anchor} fill={isWorst?T.coral:T.muted} fontSize="7.5" fontFamily="Sora" fontWeight={isWorst?"700":"400"}>{ax.l}</text>;})}
            </svg>
            <div style={{fontSize:11,color:sc,fontWeight:600}}>{score>=70?"Buena salud":score>=45?"En camino":"Necesita atención"}</div>
          </div>
          <div style={{flex:1,minWidth:200}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7,marginBottom:10}}>
              {axes.map((it)=>{const pct=it.pct;const barCol=pct>=0.8?"var(--ac)":pct>=0.4?T.amber:T.coral;const isWorst=it.key===worstItem.key;return(<div key={it.key} style={{background:isWorst?"rgba(255,95,109,.06)":T.raised,borderRadius:10,padding:"9px 11px",border:`1px solid ${isWorst?"rgba(255,95,109,.2)":T.border}`}}><div style={{fontSize:10,fontWeight:700,color:isWorst?T.coral:T.white,marginBottom:2}}>{it.l}</div><div style={{fontSize:9,color:T.muted,marginBottom:6,lineHeight:1.4}}>{it.desc}</div><div style={{display:"flex",alignItems:"center",gap:6}}><div className="prog" style={{flex:1,height:4}}><div className="progf" style={{width:`${pct*100}%`,background:barCol}}/></div><span className="mono" style={{fontSize:9,color:barCol,flexShrink:0}}>{it.v}/{it.m}</span></div></div>);})}
            </div>
            <div style={{background:"rgba(255,184,48,.06)",border:"1px solid rgba(255,184,48,.2)",borderRadius:9,padding:"8px 11px",fontSize:11,color:T.amber,lineHeight:1.5}}><span style={{fontWeight:700}}>↗ {worstItem.l}: </span>{worstItem.tip}</div>
          </div>
        </div>);
      })()}</div>
    </div>
    <div className="trend-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
      <div className="card up d3"><div style={{fontSize:12,fontWeight:600,color:T.mid,marginBottom:12}}>Últimos movimientos</div>
        {recent.length===0?<div style={{color:T.muted,fontSize:13,textAlign:"center",padding:"20px 0"}}>Sin movimientos aún</div>:recent.map(t=>(<div key={t.id} style={{display:"flex",alignItems:"center",gap:12,padding:"8px 0",borderBottom:`1px solid ${T.ink}`}}><div style={{width:32,height:32,borderRadius:9,background:T.raised,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,flexShrink:0}}>{t.category?.split(" ")[0]||"💵"}</div><div style={{flex:1,minWidth:0}}><div style={{fontSize:12,fontWeight:500,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.description}</div><div style={{fontSize:10,color:T.muted,marginTop:1}}>{t.date}</div></div><div className="mono" style={{fontSize:12,fontWeight:500,color:t.type==="income"?T.teal:t.type==="transfer"?T.muted:T.red,flexShrink:0}}>{t.type==="income"?"+":t.type==="transfer"?"↔":"-"}{fmt(t.amount)}</div></div>))}
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        <div className="card csm up d4"><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><div style={{fontSize:12,fontWeight:600,color:T.mid}}>Metas activas</div><button className="btn bg bsm" style={{fontSize:10,padding:"4px 10px"}} onClick={()=>setView("goals")}>Ver →</button></div>
          {goals.filter(g=>goalCash(g)<g.target).length===0?<div style={{color:T.muted,fontSize:12,textAlign:"center",padding:"10px 0"}}>Sin metas — <button onClick={()=>setView("goals")} style={{color:T.lime,background:"none",border:"none",cursor:"pointer",fontSize:12}}>Crear →</button></div>:
          goals.filter(g=>goalCash(g)<g.target).slice(0,4).map(g=>{const linked=holdings.filter(h=>h.goalId===g.id);const invVal=linked.reduce((s,h)=>s+calcHoldingValueArs(h,marketPrices,usdRate).curArs,0);const total=goalCash(g)+invVal;const pct=clamp((total/g.target)*100,0,100);return<div key={g.id} style={{marginBottom:9}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:12}}>{g.icon} {g.name}</span><span className="mono" style={{fontSize:10,color:T.muted}}>{pct.toFixed(1)}%</span></div><div className="prog"><div className="progf" style={{width:`${pct}%`,background:pct>=100?T.lime:pct>=60?T.blue:T.amber}}/></div></div>;})}
        </div>
...
function SalaryModule({state,update,notify}){
  ...
  <div className="kpi-grid" style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:12,marginBottom:18}}>
...
function Transactions({state,update,notify}){
  ...
  return(<div className="up">
    ...
    <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
      <select className="inp" style={{width:"auto",minWidth:100}} value={filter.month} onChange={e=>setFilter(f=>({...f,month:e.target.value}))}><option value="">Todos los meses</option>{months.map(m=><option key={m}>{m}</option>)}</select>
      <select className="inp" style={{width:"auto"}} value={filter.type} onChange={e=>setFilter(f=>({...f,type:e.target.value}))}><option value="">Todos</option><option value="expense">Gastos</option><option value="income">Ingresos</option></select>
      <select className="inp" style={{width:"auto",minWidth:100}} value={filter.cat} onChange={e=>setFilter(f=>({...f,cat:e.target.value}))}><option value="">Categorías</option>{CATS.map(c=><option key={c}>{c}</option>)}</select>
      {transferCount>0&&(
        <button onClick={()=>setShowTransfers(s=>!s)} style={{display:"flex",alignItems:"center",gap:6,padding:"8px 12px",borderRadius:9,fontSize:11,fontWeight:600,border:`1px solid ${showTransfers?"rgba(138,125,117,.4)":T.border}`,background:showTransfers?"rgba(138,125,117,.1)":T.raised,color:showTransfers?T.mid:T.muted,cursor:"pointer",transition:"all .2s",whiteSpace:"nowrap"}}>
          🔄 {showTransfers?"Ocultar":"Ver"} transferencias internas
    ...
    <div className="card" style={{padding:0,overflow:"auto"}}><table className="tbl"><thead><tr><th className="hide-m">Fecha</th><th>Descripción</th><th>Categoría</th><th className="hide-m">Tipo</th><th>Monto</th><th></th></tr></thead><tbody>
    {rows.length===0?<tr><td colSpan={6} style={{textAlign:"center",padding:"36px 0",color:T.muted,fontSize:13}}>Sin movimientos</td></tr>:rows.map(t=>(
...
function Goals({state,update,notify}){
  ...
  const addG=()=>{
    ...
    const t=Math.abs(px(form.target))*(form.currency==="USD"?usdRate:1);
    update({goals:[...goals,{id:`g_${uid()}`,name:form.name,target:t,saved:Math.abs(px(form.saved))||0,icon:form.icon,deadline:form.deadline,createdAt:todayISO(),payments:[]}]});
    ...
    {goals.length===0
      ...
      :<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(290px,1fr))",gap:14}}>
        {goals.map(g=>{
          ...
          const invCost=linkedHoldings.reduce((s,h)=>s+(h.buyPrice||0)*(h.quantity||h.qty||0)*(h.currency==="USD"?usdRate:1),0);
          ...
          return<div key={g.id} className="card up">
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
              <div style={{display:"flex",gap:10,alignItems:"center"}}><span style={{fontSize:26}}>{g.icon}</span><div><div style={{fontSize:14,fontWeight:700}}>{g.name}</div>{g.deadline&&<div style={{fontSize:10,color:T.muted,marginTop:2}}><span style={{display:"inline-flex",verticalAlign:"middle",marginRight:3}}><IcCalendar/></span>{g.deadline}{days!==null&&` · ${days>0?days+"d":"¡Hoy!"}`}</div>}</div></div>
            ...
            {pct>=100
              ?<div style={{display:"flex",gap:8}}><div style={{flex:1,background:"rgba(204,255,71,.08)",border:"1px solid rgba(204,255,71,.25)",borderRadius:10,padding:10,textAlign:"center",fontSize:13,color:T.lime,fontWeight:600}}>Alcanzada ✓</div>{(g.saved||0)<g.target&&<button className="btn bl" onClick={()=>liquidar(g,linkedHoldings,investedValue)}>Liquidar</button>}</div>
              :addTo?.id===g.id
                ?<div style={{display:"flex",flexDirection:"column",gap:7}}>
                    ...
                    <div style={{display:"flex",gap:7}}><input className="inp" style={{flex:1,fontSize:12}} placeholder="Monto" value={addAmt} onChange={e=>setAA(e.target.value)} autoFocus onKeyDown={e=>e.key==="Enter"&&addSav(g)}/><select className="inp" style={{width:70,fontSize:12,padding:"10px 6px"}} value={addCur} onChange={e=>setAC(e.target.value)}><option>ARS</option><option>USD</option></select><button className="btn bl bsm" onClick={()=>addSav(g)}>+</button><button className="btn bg bsm" onClick={()=>{setAT(null);setAA("");setAL("");}}>✕</button></div>
                  </div>
                :<div style={{display:"grid",gridTemplateColumns:holdings.length>0?"1fr 1fr 1fr":"1fr 1fr",gap:7}}>
...
function Investments({state,update,notify}){
  const {savedAnalyses=[],riskProfile,goals=[],usdRate}=state;
  const {fmt}=useDsp(state);
  ...
  const refreshPortfolio=async()=>{
    ...
    for(const h of holdings){
      if(h.type==="crypto"){try{const r=await fetch(`https://api.binance.com/api/v3/ticker/price?symbol=${h.ticker.toUpperCase()}USDT`);if(r.ok){const d=await r.json();newPrices[h.ticker]={price:parseFloat(d.price),currency:"USD"};updated++;}}catch(e){}}
      else if(["accion","cedear","etf"].includes(h.type)){const qt=(h.type==="cedear"||h.originalCurrency==="ARS")&&!h.ticker.endsWith(".BA")?`${h.ticker}.BA`:h.ticker;const pd=await fetchStockPrice(qt);if(pd?.price){newPrices[h.ticker]={price:pd.price,currency:pd.currency||"USD"};updated++;}}
    }
  ...
  const addHolding=()=>{
    ...
    setSHF(false);setHF({type:"accion",ticker:"",name:"",quantity:"",buyPrice:"",totalInvested:"",currency:"ARS",buyDate:todayISO(),maturityDate:"",rate:"",goalId:""});
  ...
  const runScanner=async()=>{
    ...
    try{const r=await autoScanInvestments(riskProfile,usdRate);
      if(r?.opportunities){const pr=await Promise.all(r.opportunities.map(o=>fetchStockPrice(o.ticker)));const np={...marketPrices};r.opportunities=r.opportunities.map((o,i)=>{const pd=pr[i];if(pd?.price){o.currentEstimate=pd.price;np[o.ticker]={price:pd.price,currency:pd.currency||"USD"};}return o;});update({marketPrices:np});}
      setSR(r);notify(`${r.opportunities?.length||0} oportunidades ✓`);
  ...
  const analyze=async(t,n)=>{
    ...
    if(ex){setSel(ex);const mp=state.marketPrices[t];if(mp?.price)setHF(f=>({...f,ticker:t,name:n||t,buyPrice:String(mp.price),currency:mp.currency||"USD",type:"accion"}));return;}
    setLoad(true);setLE(null);
    const[r,pd]=await Promise.all([analyzeStock(t,n||t),fetchStockPrice(t)]);
    if(r){const np={...state.marketPrices};if(pd?.price){r.currentEstimate=pd.price;np[t]={price:pd.price,currency:pd.currency||"USD"};setHF(f=>({...f,ticker:t,name:n||r.company||t,buyPrice:String(pd.price),currency:pd.currency||"USD",type:"accion"}));}update({savedAnalyses:[r,...savedAnalyses.filter(a=>a.ticker!==r.ticker)],marketPrices:np});setSel(r);}
  ...
  return(<div className="up">
    ...
    {tab==="portfolio"&&<div>
      ...
      {showHForm&&<div className="card" style={{marginBottom:14}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:14}}><div style={{fontSize:14,fontWeight:700}}>Nueva Inversión</div><button className="btn bg bsm" onClick={()=>setSHF(false)}><ic.X/></button></div>
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          <div><label style={{fontSize:11,color:T.muted,display:"block",marginBottom:5}}>Tipo</label><select className="inp" value={hForm.type} onChange={e=>{const t=e.target.value;let ac=hForm.currency;if(["accion","cedear","plazo_fijo","fci","bono"].includes(t))ac="ARS";if(t==="crypto")ac="USD";setHF(f=>({...f,type:t,currency:ac}));}}><option value="accion">📈 Acción</option><option value="cedear">🇺🇸 CEDEAR</option><option value="etf">📊 ETF</option><option value="crypto">₿ Crypto</option><option value="plazo_fijo">🏦 Plazo fijo</option><option value="fci">💼 FCI</option></select></div>
          {["accion","cedear","etf","crypto"].includes(hForm.type)?<>
            ...
            <div className="g3"><div><label style={{fontSize:11,color:T.muted,display:"block",marginBottom:5}}>Cant.</label><input className="inp" value={hForm.quantity} onChange={e=>setHF(f=>({...f,quantity:e.target.value}))}/></div><div><label style={{fontSize:11,color:T.muted,display:"block",marginBottom:5}}>Precio Unit.</label><input className="inp" value={hForm.buyPrice} onChange={e=>setHF(f=>({...f,buyPrice:e.target.value}))}/></div><div><label style={{fontSize:11,color:T.muted,display:"block",marginBottom:5}}>Moneda</label><select className="inp" value={hForm.currency} disabled={["accion","cedear","crypto"].includes(hForm.type)} style={{opacity:["accion","cedear","crypto"].includes(hForm.type)?0.6:1}} onChange={e=>setHF(f=>({...f,currency:e.target.value}))}><option value="ARS">ARS</option><option value="USD">USD</option></select></div></div>
          </>:<>
            <div className="g2"><div><label style={{fontSize:11,color:T.muted,display:"block",marginBottom:5}}>Banco / Entidad</label><select className="inp" value={hForm.name} onChange={e=>{const s=BANCOS_RATES.find(b=>b.name===e.target.value);setHF(f=>({...f,name:e.target.value,rate:s&&s.tna?s.tna:f.rate}));}}><option value="">Seleccionar banco...</option>{BANCOS_RATES.map(b=><option key={b.id} value={b.name}>{b.name}{b.tna?` (${b.tna}%)`:""}</option>)}</select></div><div><label style={{fontSize:11,color:T.muted,display:"block",marginBottom:5}}>TNA %</label><input className="inp" value={hForm.rate} onChange={e=>setHF(f=>({...f,rate:e.target.value}))} placeholder="35.5"/></div></div>
        ...
        {portfolioData.items.map(h=>{
          ...
          return(<div key={h.id} className="card" style={{padding:"14px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div><div style={{fontSize:14,fontWeight:700}}>{h.ticker||h.name} <span style={{fontSize:10,color:T.muted,fontWeight:400}}>{h.type}</span></div><div style={{fontSize:11,color:T.muted}}>{h.quantity?`${h.quantity} un. a ${symDisplay}${buyPriceDisplay.toLocaleString("en-US")}`:`TNA ${h.rate}%`}{cmp&&<span style={{marginLeft:8,color:T.mid}}>· actual: {cmp.currency==="USD"?fUSD(cmp.price):fARS(cmp.price)}</span>}</div></div>
              <div style={{display:"flex",gap:6}}>
                {["accion","cedear","etf","crypto"].includes(h.type)&&<button className="btn bg bsm" style={{color:T.lime,padding:"4px 8px"}} onClick={()=>autoRefreshSingle(h)} disabled={refreshingId===h.id}>{refreshingId===h.id?<Dots/>:<ic.Refresh/>}</button>}
                <button className="btn bg bsm" style={{padding:"4px 8px"}} onClick={()=>editPriceManual(h)}>✎</button>
                <button className="btn bd bsm" style={{padding:"4px 8px"}} onClick={()=>delHolding(h.id)}><ic.Trash/></button>
              </div>
            </div>
          ...
          {scanResult.opportunities?.map((opp,i)=>(
            <div key={i} className="card" style={{cursor:"pointer",border:`1px solid ${opp.ticker===scanResult.topPick?T.lime:T.border}`,background:opp.ticker===scanResult.topPick?"rgba(200,255,87,.03)":T.surface,position:"relative",transition:"all .2s"}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                <div><div style={{display:"flex",gap:7,alignItems:"center",marginBottom:3}}><span className="mono" style={{fontSize:16,fontWeight:700}}>{opp.ticker}</span><span className={`tag ${sigCls(opp.signal)}`} style={{fontSize:10}}>{opp.signal}</span></div><div style={{fontSize:11,color:T.muted}}>{opp.name}</div>{opp.currentEstimate>0&&<div className="mono" style={{fontSize:11,color:T.mid,marginTop:2}}>${opp.currentEstimate.toLocaleString("en-US",{maximumFractionDigits:2})}</div>}</div>
              </div>
    ...
    {tab==="manual"&&<div>
      ...
      {sel&&<div><AnalysisDetail a={sel} onClose={()=>setSel(null)}/>{sel.currentEstimate>0&&<div style={{marginTop:10,background:"rgba(200,255,87,.06)",border:`1px solid rgba(200,255,87,.2)`,borderRadius:12,padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,flexWrap:"wrap"}}><div style={{fontSize:12,color:T.mid}}>Precio real: <span className="mono" style={{color:T.lime}}>{sel.currentEstimate>1000?fARS(sel.currentEstimate):fUSD(sel.currentEstimate)}</span></div><button className="btn bl bsm" onClick={()=>{setTab("portfolio");setSHF(true);setSel(null);}}>+ Agregar al portfolio →</button></div>}</div>}
    </div>}
    ...
    {tab==="saved"&&<div>
      {savedAnalyses.length===0?<div style={{textAlign:"center",padding:"48px 32px",color:T.muted}}><div style={{width:52,height:52,margin:"0 auto 14px",background:"rgba(91,158,255,.08)",borderRadius:16,display:"flex",alignItems:"center",justifyContent:"center",color:T.blue}}><IcSaved/></div><div style={{fontSize:14}}>Sin análisis guardados</div></div>:(
        <div><div className="inv-grid" style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(250px,1fr))",gap:10,marginBottom:14}}>
          {savedAnalyses.map(a=>(
        ...
        </div>{sel&&<AnalysisDetail a={sel} onClose={()=>setSel(null)}/>}</div>
      )}
    </div>}
  </div>);
}
...
function Analytics({state,update,setView}){
  ...
  const cm={};
  transactions.filter(t=>t.type==="expense").forEach(t=>{cm[t.category]=(cm[t.category]||0)+t.amount;});
  ...
  let totInc=0;
  allMonths.forEach(m=>{
    totInc+=getSalaryTotal(salaries,m);
    totInc+=transactions.filter(t=>gMonth(t.date)===m&&t.type==="income"&&t.source!=="salary").reduce((s,t)=>s+t.amount,0);
  });
  ...
  return(<div className="up">
    <PH title="Analíticas" sub="Histórico · Proyecciones · Patrimonio" right={<div style={{display:"flex",gap:8,alignItems:"center"}}>{setView&&<button className="btn bg bsm" onClick={()=>setView("salary")}><ic.Salary/>Editar sueldo</button>}<select className="inp" style={{width:"auto"}} value={range} onChange={e=>setRange(+e.target.value)}><option value={3}>3 meses</option><option value={6}>6 meses</option><option value={12}>12 meses</option></select></div>}/>
    ...
    {(portfolioValArs>0||totalSavingsArs>0)&&
      <div className="kpi-grid" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:14}}>
        {[{l:"Portfolio",v:fmt(portfolioValArs),c:T.blue,i:<IcPortfolio/>},{l:"P&L Portfolio",v:`${portfolioPnlArs>=0?"+":""}${fmt(portfolioPnlArs)}`,c:portfolioPnlArs>=0?T.teal:T.red,i:portfolioPnlArs>=0?"📈":"📉"},{l:"Ahorros (Efectivo)",v:fmt(totalSavingsArs+goalPaymentsArs),c:T.teal,i:<IcFree/>},{l:"Patrimonio Total",v:fmt(portfolioValArs+totalSavingsArs),c:T.lime,i:"🏛️"}].map((k,i)=>(
    ...
    <div className="kpi-grid" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:16}}>
      {[{l:"Total ingresos",v:fmt(totInc),c:T.teal},{l:"Total gastos",v:fmt(totExp),c:T.red},{l:"Tasa de ahorro",v:`${savR}%`,c:savN>=20?T.lime:savN>=10?T.amber:T.red,sub:savN>=20?"✓ Excelente":savN>=10?"Regular":savN<0?"⚠ Gastás más de lo que ingresás":"⚠ Mejorar"}].map((k,i)=>(
    ...
    <div className="trend-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
      <div className="card">
        ...
        {cats.length===0?<div style={{color:T.muted,textAlign:"center",padding:20,fontSize:13}}>Sin datos</div>:cats.slice(0,8).map((c,i)=>(
          <div key={i} style={{marginBottom:9}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:12,color:T.mid}}>{c.c}</span><span className="mono" style={{fontSize:11,color:T.muted}}>{c.pct}%</span></div><div className="prog" style={{height:4}}><div style={{height:"100%",borderRadius:2,background:c.col,width:`${clamp(c.pct,0,100)}%`,transition:"width .6s"}}/></div></div>
        ))}
      </div>
      <div className="card">
...
function Import({state,update,notify}){
  ...
  const extractImg=async()=>{
    ...
    if(!result||!result.transactions?.length){setExt("error");setEE("No se detectaron transacciones.");notify("Sin transacciones detectadas","err");return;}
  ...
  const handleCSV=file=>{
    const r=new FileReader();
    r.onload=e=>{const p=parseCSV(e.target.result);if(!p.length)return notify("Sin datos válidos en el CSV","err");setPreview(p);notify(`${p.length} movimientos detectados`);};
  ...
  const handlePDF=async file=>{
    ...
    try{
      ...
      if(!results.length){setExt("error");setEE("No se detectaron transacciones en el PDF.");notify("Sin transacciones detectadas","err");}
      else{setPreview(results);setExt("done");notify(`${results.length} transacciones extraídas del PDF ✓`);}
  ...
  const autoCatAll=async()=>{
    ...
    const upd=[];
    for(const t of preview){const r=await autoCat(t.description);upd.push({...t,category:r.category||t.category,type:r.type||t.type});}
          ...
          <tbody>
            {preview.map(t=>(
              <tr key={t.id}>
                ...
                <td><select className="inp" style={{fontSize:11,padding:"4px 8px"}} value={catE[t.id]||t.category} onChange={e=>setCE(c=>({...c,[t.id]:e.target.value}))}>{CATS.map(c=><option key={c}>{c}</option>)}</select></td>
              </tr>
            ))}
          </tbody>
        </table>
              ...
              <div key={i} style={{background:T.raised,borderRadius:12,padding:"14px 16px",marginBottom:10,border:`1px solid ${T.border}`}}>
                <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:12}}>
                  <div style={{display:"flex",justifyContent:"space-between"}}>
                    ...
                    <span className="mono" style={{fontSize:12,color:T.teal}}>+{fARS(inc.amount)}</span>
                  </div>
                  <div style={{display:"flex",justifyContent:"space-between"}}>
                    <span style={{fontSize:12,color:T.coral}}>↓ {exp.description}</span>
                    <span className="mono" style={{fontSize:12,color:T.coral}}>-{fARS(exp.amount)}</span>
                  </div>
                ...
                <div style={{display:"flex",gap:8}}>
                  <button className="btn bl bsm" style={{flex:1,justifyContent:"center"}} onClick={()=>{
                    update({transactions:state.transactions.map(t=>t.id===pair.a.id||t.id===pair.b.id?{...t,type:"transfer",category:"🔄 Transferencia interna"}:t)});
                    setRejectedPairs(r=>new Set([...r,pair.a.id+pair.b.id]));
                    notify("Transferencia interna marcada ✓");
                  }}>✓ Sí, es una transferencia</button>
                  <button className="btn bg bsm" style={{flex:1,justifyContent:"center"}} onClick={()=>setRejectedPairs(r=>new Set([...r,pair.a.id+pair.b.id]))}>No, son distintos ✗</button>
                </div>
              </div>
            );
          })}
