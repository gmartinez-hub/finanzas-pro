...
export default function AppRoot({defaults,components,icons,getHealth,legacyStyles,supabaseConfig}){
  ...
  const mainRef=useRef(null),toastTimer=useRef(null),lastWrite=useRef(true),menuButton=useRef(null);
  ...
  return <div className="mangos-app"><style>{legacyStyles}</style>
    ...
    {rateOpen&&<RateDialog state={state} update={update} notify={notify} onClose={()=>setRateOpen(false)}/>}
...
function RateDialog({state,update,notify,onClose}){
