...
**Spec:** `/Users/gabo/Documents/ChatGPT/Logbook/auditorias/mangos-2026-09-16/Alcance-proxima-entrega.md`.
...
- [ ] Commit recovery separately before feature work.
...
- [ ] Write tests asserting negative Argentine money (`-1.234,56`), salary+extra counted once, reserve/payment distinction, equivalent partial month comparison, simulation immutability, recurring confirmation/link idempotency, goal payment/reversal relationships and canonical ARS semantics.
- [ ] Run `node --test tests/finance.test.js` and observe expected failures before implementation.
- [ ] Implement pure functions with validation and explicit unsupported/ambiguous states. Round money through centavos, preserve original metadata, do not infer account balances.
- [ ] Pass domain tests and report exact exported interfaces and integration requirements.
...
- [ ] Tests: quota error reaches caller, corrupt state is preserved and not replaced, invalid restore leaves old data, round-trip backup, repeated same import is idempotent, duplicate legitimate rows within one batch preserved, Spanish signs/types/categories round-trip.
- [ ] Run focused node tests and capture expected failures.
- [ ] Implement adapters, validation and reversible restoration. No database migration or network code.
- [ ] Pass tests and document integration surface for the root agent.
...
- [ ] Integrate domain and storage modules. Remove automatic recurrence writes on mount. Preserve data after failed load and show persistent save errors/conflicts.
- [ ] Implement warm/cold app shell, detailed mango, four main routes and accessible secondary tools. Keep all existing feature routes reachable.
- [ ] Overview: available/month result with definition and data limitation, income/spending/reserved breakdown, substantial monthly bars and main goal. Clicks open filtered transactions. Changes use comparable periods and no mandatory model request.
- [ ] Pending payments: confirm/create once, link an existing movement, skip/pause without modifying paid history. Never presume a due date.
- [ ] Decision dialog: amount and gasto/reserva; before/after, no writes. Native dialog focus/Escape and visible validation.
- [ ] Settings: complete backup/download and restore preview/confirmation; raw recovery download on load failure. Demo selected explicitly, isolated local key, resettable synthetic scenario and no live price/AI calls.
- [ ] Replace vulnerable dynamic PDF reader with pinned maintained package/configuration or disable only that format with a clear reason if verification fails.
- [ ] Test integration invariants and run lint/build. Browser walkthrough at desktop and mobile with synthetic data, using actual UI controls.
...
- [ ] Test all legacy slide indexes map to intentional semantic steps, including closing slide 23; steps target real reachable controls or explanations, not arbitrary DOM clicks.
- [ ] Implement opt-in tour, initial-state read, update subscription scoped to live row, reconnect read, cleanup, previous/next local controls and responsive tooltip positioning. Highlight/open actions only; no automatic payment or AI generation.
- [ ] Preserve needed navigation/panel opening through a small event/route contract. Date/currency selection remains deterministic; USD step explicitly selects USD.
- [ ] Update connected presentation thermal visual tokens, copy and start/stop state controls; remove automatic publishing on load. Keep offline/local navigation usable.
- [ ] Verify 12 steps against new app anchors, including empty-state fallbacks, desktop and mobile. Use mocked transport; no live state mutation in QA.
...
- [ ] Fix Node/pnpm versions and a single lockfile. `pnpm test`, `pnpm lint`, `pnpm build` on the final code.
- [ ] Independent review of money/storage/tour integration and preservation of recovered work; fix concrete findings and repeat only covering checks.
